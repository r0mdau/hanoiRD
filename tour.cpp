...
{
    //on prend le numéro du nombre de plateaux comme le numéro du plus gros plateau
    //(+le numéro du plateau est gros, + il est gros)
...
void Tour::deplacerPlateau(Tour *p_tourArrivee)
{
    ...
    else if(p_tourArrivee->getPile().empty() || pilePlateaux.top().puisJeEtreDeposerSurCePlateau(p_tourArrivee->getPile().top()))
    {
        ...
        pilePlateaux.pop(); //on enleve le plateau que l'on vient de déplacer
    }else
